% % % photrack AG, June-November 2012, Beat Lüthi
% % %
% % % this script is the main for job:
% % % customer: PTV 2 cameras
...
%%%%%%%%%% CONSTANTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%camera stuff:
pix=[512 1024];                                  %(1,1)=horizontal=x, (1,2)=vertical(bottom->top)= y
dp=20e-6;                                        %[m]

%recording specific stuff
fps=2000;
num_frames=2000;
max_points_per_frame=900;
path='C:\Users\Luethi\Documents\Dropbox\PTV_working_folders\';

exp=['2000fps_1Hz_rec4'];
img_folder=['img_1Hz_rec4/'];
res_folder=['res_1Hz_rec4/'];
cal_folder=['cal1/'];
cal_name{1}=[path,cal_folder,'cal1_cam1'];
cal_name{2}=[path,cal_folder,'cal1_cam2'];

%calibration
poly_order=3; 

%domain and flow params
domain_radius=0.05; %[m]
hole_radius=0.005; %[m]
Hz=1;%3.48;
domain_width=2*domain_radius;
max_vel=3.0;                                     %[m/s] <-----TUNING
tol_vz=0.9;                                      %[m/s] <-----TUNING
...
if exist(cam_const_name)%1>2%
    ...
    ind=cal(:,1);
    cal=cal(:,2:4)*1e-3;
    corners=[find(ind==1) find(ind==7) find(ind==38) find(ind==40) find(ind==141) find(ind==147) ];
    ...
    axis([-0.04 0.04 -0.11 0.11 -0.04 0.04])
    view(16,66)
...
if exist(points_name)%1>2%
    disp('loading determined 3d points')
    load(points_name)
    tmp=size(points);
            ...
            if points(1,frame,pid1,4)==0 %% not already occupied
                ...
                if le>0
                    ...
                    for i=1:le
                        ...
                        if dist<tol
                            if (points3D(1,1)^2+points3D(1,3)^2)^0.5<domain_radius % is it within domain?
                                pid2=cand_list(i);
...
% %now we should/can? generously glue the trajectories
